...
This is a literate source file; the source code can be extracted with ~org-babel-tangle~. This is bootstrapped in [[file:init.el][init.el]].

Some of this was inspired by [[https://github.com/patrickt/emacs][this configuration]].
...
I don't like accidentally calling ~mark-page~, and I never intend to
call it. Let's unbind it.
#+begin_src emacs-lisp
...
I would like that same arrangement when automatic splitting happens.
The ~split-window-sensibly~ function almost does what we want, but it
tries to split vertically before splitting horizontally. (If we simply
set ~split-width-threshold~ to ~0~ and ~split-height-threshold~ to
~nil~, we make too many columns on consecutive splits.)

#+begin_src emacs-lisp
  (defun split-window-more-sensibly (&optional window)
    "Split WINDOW in a way suitable for `display-buffer'.
  See `split-window-sensibly' for the original implementation."
...
We can now use ~split-window-more-sensibly~ as the splitter.
...
Require that files contain a trailing newline.
#+begin_src emacs-lisp
  (setq require-final-newline t)
#+end_src

Create a function to disable visible trailing whitespace.
...
#+begin_src emacs-lisp
  (letrec ((go (lambda (fonts)
    ...
    (funcall go '(
                  ...
                  "FiraCode Nerd Font Mono 14"
                  "FiraMono 14"
                  "NotoSansM Nerd Font 14"
                  "SF Mono 14")))
#+end_src
...
pgformatter (SQL):
#+begin_src emacs-lisp
  (defcustom sql-command
    "pg_format"
    "The command to run when applying sql formatting"
    :type 'string
    :safe 'stringp
    :local 't
    :group 'prsteele)
...
If we're compiling something that tries to clear the screen, do so.
This helps with "watching" output.

#+begin_src emacs-lisp
  (defun clear-in-region-p (text)
    (string-match-p (regexp-quote "[J") text))

  (defun clear-comint-hook (text)
    (if (clear-in-region-p text)
        (comint-clear-buffer))
    text)
#+end_src
...
#+begin_src emacs-lisp
  (use-package compile
    ...
    :config
    (add-hook 'comint-preoutput-filter-functions #'clear-comint-hook))
#+end_src
...
#+begin_src emacs-lisp
  (use-package docker-compose-mode
    ...
    :hook
    (docker-compose-mode . (lambda () (auto-fill-mode -1))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package eldoc-box
    :hook
    (eldoc-box-buffer . disable-trailing-whitespace))
#+end_src
...
#+begin_src emacs-lisp
  (use-package j-mode
    ...
    (custom-set-faces
     '(j-verb-face ((t (:foreground "#f38ba8"))))
     '(j-adverb-face ((t (:foreground "#f9e2af"))))
     '(j-conjunction-face ((t (:foreground "#cba6f7"))))
     '(j-other-face ((t (:foreground "#89b4fa")))))
    :hook
    (j-mode . (lambda ()
...
We define a helpful function for inserting pairs of code-example
imports in Org.

#+begin_src emacs-lisp
  (defun insert-code-output-pair (file type)
    (interactive "fFile: \nMtype: ")
    (let* ((rel-file (f-relative file))
           (rel-target (f-relative (f-join "out" (s-concat (f-filename file) ".out"))))
           (source (s-concat "#+INCLUDE: \"" rel-file "\" src " type))
           (dest (s-concat "#+INCLUDE: \"" rel-target "\" example")))
      (insert source)
      (insert "\n")
      (insert dest)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package prog-mode
    :after (eglot rainbow-delimiters rainbow-identifiers)
    :bind
    (:map prog-mode-map
          ("C-." . 'xref-find-definitions)
...
#+begin_src emacs-lisp
  (use-package sql
    ...
    :config
    (add-to-list 'auto-mode-alist '("\\.duckdb" . sql-mode))
    :hook
    (sql-mode . sql-format-on-save-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package uiua-ts-mode
    ...
    (uiua-ts-mode . (lambda ()
                      ...
                      (buffer-face-set '(:family "Uiua386"))
...
Automatically clean up trailing whitespace, /iff/ the buffer was
originally clean.
...
#+begin_src emacs-lisp
  (use-package yaml-ts-mode
    :hook
    (yaml-ts-mode . (lambda () (setq tab-width ))))
#+end_src
